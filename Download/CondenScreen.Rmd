...
# ---- Install Required Packages (Uncomment and run chunk if needed) ----
...
#############################################################
save_Location <- "D:/CRISPR_Screen_Analysis_dp/Pharmakon1760/updated_analysis_dp_3_6_25/results/"
#############################################################
...
#############################################################
file_paths <- dir(file_Location, pattern = "*.shape_size_inensity", full.names = TRUE)
file_paths2 <- dir(file_Location, pattern = "*.quantified_foci", full.names = TRUE)
#############################################################

# Save file paths to disk so each batch can access them
saveRDS(file_paths, paste0(file_Location, "file_paths.rds"))
saveRDS(file_paths2, paste0(file_Location, "file_paths2.rds"))
...
#================================================================
  ...
  # Process rows where plate_Num is NA or "READ" 
  mask <- is.na(result$plate_Num) | result$plate_Num == "READ"
  
  if (any(mask)) {
    # Only process the necessary rows from PathName_GFP
    matches_path <- str_match(combinedDF$PathName_GFP[mask], pattern_pathname)
    result$plate_Num[mask] <- matches_path[, 2]
  }
...
combined_data$plate_Num <- extractedInfo[,1]
combined_data$Well_Num <- extractedInfo[,2]
combined_data$Imagecopy_Num <- extractedInfo[,3]
combined_Df <- combined_data %>% filter(complete.cases(.))
...
perImageStats <- combined_Df %>%
  group_by(plate_Num, Well_Num, Imagecopy_Num) %>%
  summarise(
    Nuclei_Number = n(), 
    mean_Nuc_Foci = mean(Children_GFP_foci_Count)
  )


colnames(dfSelection)[8] <- "Well_Num"
combined_Df  <- merge(combined_Df, perImageStats)
...
pos_Ctrl_DF <- combined_Df %>% group_by(plate_Num) %>%
  ...
  summarize(mean_Pos_SD = sd(Avg_Foci), mean_Pos_Foci = mean(Avg_Foci), cv_Pos = cv(Avg_Foci))

neg_Ctrl_DF <- combined_Df %>% group_by(plate_Num) %>%
  filter( Condition == neg_Ctrl) %>%
  summarize(mean_Neg_SD = sd(Avg_Foci), mean_Neg_Foci = mean(Avg_Foci), cv_Neg= cv(Avg_Foci))
...
SB_Plate <- plate_Summary %>% group_by(plate_Num) %>%
  mutate(
    SB_Plate = ifelse(
      unique(combined_Df$`AssayType`) == "OFF", 
...
#EXPORT PLATE SUMMARY page HERE
...
for (plate in seq_len(length(unique(combined_Df$plate_Num)))){
  ...
  ggsave(filename = paste0(sub_dir, paste(plate_List[plate], "_Figure.png")), plot = combined_plot, width = 8, height = 8)
...
colnames(plate_Summary) <- c("Plate", "STD Pos. Ctrl", "Avg. Foci Pos. Ctrl", "CV Pos. Ctrl","STD Neg. Ctrl", "Avg. Foci Neg. Ctrl", "CV Neg. Ctrl", "Plate Z'-Score", "Signal-to-Background")
...
colnames(ctrls) <- c("Plate", "Control", "Cell Count", "Avg. Foci", "Nuclear Area", "Nuclear FormFactor")
...
if(assayType == "OFF"){
  ...
  colnames(combined_Df) <- c("BZScore", "Viability", "Plate", "Well", "Plate Z'-Score", "%Effect", "Avg. Foci", "%Diff (Cell Count)", "Cell Count", "ZScore", "B-Score", "%Diff (Nuclear Area)", "%Diff FormFactor", "Assay Type")
...
if(assayType == "ON"){
  ...
  colnames(combined_Df) <- c("ZScore", "Viability", "Plate", "Well", "Plate Z'-Score", "%Effect", "Avg. Foci", "%Diff (Cell Count)", "Cell Count",  "%Diff (Nuclear Area)", "%Diff FormFactor", "Assay Type")
...
export_File <- paste0(file_Location, "CondenScreen.xlsx")
wb <- createWorkbook()

# Check if output file exists, create if needed
...
# Write to Excel
writeData(wb, sheet = "Combined Analysis", x = combined_Df, startCol = 1, startRow =  1)
writeData(wb, sheet = "Plate Statistics", x = plate_Summary, startCol = 1, startRow = 1)
writeData(wb, sheet = "Controls", x = ctrls, startCol = 1, startRow =  1)
...
#EDIT: Change to location of a file linking compound/gene names to each well, if applicable, else comment out. 
#############################################################
if (unique(merged_df$'Assay Type') == "OFF") {
  template_location <- paste0(file_Location, "20240730_MSPharmakon_All Results_YCMD_dp EditsV2.xlsx")
  template <- read.xlsx(template_location, sheet = "Sheet 1")
  colnames(template) <- c("Plate", "Well", "Reference")

 } else {
  template <- read.xlsx(paste0(file_Location, "20231221_Schlieker WT CRISPR Screen_Report.xlsx"))
...
genes_of_interest <- c("APOL3", "ZNF335", "RNF26", "RAN", "NUP50", "EIF4A3", "TOR1A", "TOR1B")
#------------------------------------------------------------------------
...
merged_df$Average_foci_perCell <- as.numeric(merged_df$Avg..Foci)
merged_df$Normalized_Effect <- as.numeric(merged_df$"%Effect")


# Merge data with compound names
merged_df <- merge(merged_df, template[, c("Plate", "Well", "Reference")], 
                   by = c("Plate", "Well"), all = TRUE)
merged_df <- merged_df[!is.na(merged_df$Reference), ]  # Keep only rows with compound names
...
plot_normalized_score <- function(df, assay_type, image_path, compounds_of_interest, genes_of_interest) {
  ...
  p <- ggplot(df, aes(x = paste0(Well, Plate), y = !!score_col,
    ...
    geom_hline(yintercept = 2, color = "darkred", linetype = "dashed", size = 0.5) +
    geom_text_repel(data = filter(df, !!score_col >= 3 & !Reference %in% genes_of_interest |!!score_col >= 3 & !Reference %in% compounds_of_interest ),
                    aes(label = Reference), box.padding = 0.4, size = 3.5,
    ...
    geom_text_repel(data = filter(df, Reference %in% genes_of_interest | Reference %in% compounds_of_interest),
                    aes(label = Reference), box.padding = 3.5, size = 6,
                    max.overlaps = Inf, fontface = "bold", color = "red") +
    guides(fill = "none") +
    coord_cartesian(ylim = y_limits)

  # Save the plot
  ggsave(paste0(image_path, "normalized_SigScore.pdf"),
         p, dpi = 300, width = 10, height = 10)
  print(p)
  return(p)
}

#Change the second variable (assay_type) to be either "OFF" or "ON" for a signal-off or signal-on screen readout, respectively:
...
# Create histogram of normalized effect
  normalized_dist <- ggplot(merged_df, aes(x = Normalized_Effect)) +
    geom_histogram(color = "black", fill = "darkred", binwidth = 1) +
    geom_density(alpha = 0.6, fill = "darkgray") +
    # Add reference lines and labels
    geom_vline(aes(xintercept = 100), linetype = 'dashed', size = 1.75) +
    geom_vline(aes(xintercept = 0), linetype = 'dashed', size = 1.75) +
    theme_classic() +
    theme(title = element_text(size = 40, face = "bold"),
          axis.title.x = element_text(size = 30, face = "bold"),
          axis.text.y = element_text(size = 30, face = "bold"),
...
normalized_dist <- normalized_dist +
  annotate("text", x = 90, y = 45, label = pos_Ctrl, angle = 90, size = 12) +
  annotate("text", x = -13, y = 45, label = neg_Ctrl, color = "black", angle = 90, size = 12) +
  labs(title = NULL, y = "Count", x = "% Decrease in Foci")
 } else {
normalized_dist <- normalized_dist +
  annotate("text", x = 90, y = 45, label = neg_Ctrl, angle = 90, size = 12) +
  annotate("text", x = -13, y = 45, label = pos_Ctrl, color = "black", angle = 90, size = 12) +
  labs(title = NULL, y = "Count", x = "% Increase in Foci")
 } 
     
  # Save distribution plots
  ggsave(paste(image_Save_Location, "distrpilot.pdf", sep = ""), 
        normalized_dist, 
        dpi = 300, 
        width = 10, 
        height = 10)
  
  ggsave(paste(image_Save_Location, "rawdistrpilot.pdf", sep = ""), 
        raw_distribution, 
        dpi = 300, 
        width = 10, 
        height = 10)
...
```{r}
#Convert variables to original naming system:
if(assayType == "OFF"){
  merged_df <- merged_df[, c("BZScore", "Viability", "Plate", "Well", "Plate.Z'-Score", "%Effect", "Avg..Foci", "%Diff.(Cell.Count)", "Cell.Count", "ZScore", "B-Score", "%Diff.(Nuclear.Area)", "%Diff.FormFactor",  "Assay.Type", "Reference") ]

  colnames(merged_df) <- c("BZScore", "Viability", "Plate", "Well", "Plate Z'-Score", "%Effect", "Avg. Foci", "%Diff (Cell Count)", "Cell Count", "ZScore", "B-Score", "%Diff (Nuclear Area)", "%Diff FormFactor", "Assay Type", "Reference")
...
if(assayType == "ON"){
  merged_df <- merged_df[, c("ZScore", "Viability", "Plate", "Well", "Plate.Z'-Score", "%Effect", "Avg..Foci", "%Diff.(Cell.Count)", "Cell.Count", "%Diff.(Nuclear.Area)", "%Diff.FormFactor",  "Assay.Type", "Reference") ]

  colnames(merged_df) <- c("ZScore", "Viability", "Plate", "Well", "Plate Z'-Score", "%Effect", "Avg. Foci", "%Diff (Cell Count)", "Cell Count",  "%Diff (Nuclear Area)", "%Diff FormFactor", "Assay Type", "Reference")
...
writeData(wb, sheet = "Combined Analysis", x = merged_df, startCol = 1, startRow =  1)
