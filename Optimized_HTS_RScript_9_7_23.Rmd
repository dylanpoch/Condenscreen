...
save_Location <- "C:/Users/Schlieker Lab/Desktop/Dylan/Compound Screen/pilot_screen_9_1_23_dp/pilot_24h_results.xlsx"
...
plate_layout <- df %>%
  mutate(condition = case_when(
      Well_Location %in% c("A01", "A02", "B01", "B02", "C01", "C02", "D01", "D02", "E01", "E02", "F01", "F02", "G01", "G02", "H01", "H02", "I01", "I02", "J01", "J02", "K01", "K02", "L01", "L02", "M01", "M02", "N01", "N02", "O01", "O02") ~ paste0("DMSO (0.1%)_", well_Number),
      #Well_Location %in% c("A02", "B02",  "C02", "D02",  "E02", "F02", "G02",  "H02", "I02", "J02",  "K02", "L02", "M02", "N02", "O02") ~ "DMSO (0.1%)",

      Well_Location %in% c("A23", "B23", "C23", "D23", "E23", "F23", "G23", "H23", "I23", "J23", "K23", "L23", "M23", "N23", "O23") ~ paste0("OSMI-1 (50 uM)_", well_Number),
      Well_Location %in% c("A24", "B24", "C24", "D24", "E24", "F24", "G24", "H24", "I24", "J24", "K24", "L24", "M24", "N24", "O24") ~ paste0("WT Ctrl_", well_Number),
...
DMSO_Negative_Ctrl <- na.omit(per_Well_Plate[per_Well_Plate$condition == paste0("DMSO (0.1%)_", well_Number), ])
OSMI1_50uM_Ctrl <-  na.omit(per_Well_Plate[per_Well_Plate$condition == paste0("OSMI-1 (50 uM)_", well_Number), ])
WT_Ctrl <-  na.omit(per_Well_Plate[per_Well_Plate$condition == paste0("WT Ctrl_", well_Number), ])

per_Well_Plate$conf_Location <- per_Well_Plate$Well_Location

rows_to_Delete <- c()
#Only use this code for posters/presentations where information is confidential 
for (i in 1:length(per_Well_Plate$Well_Location)){
  if(per_Well_Plate$condition[i] != paste0("DMSO (0.1%)_", well_Number) & per_Well_Plate$condition[i] != paste0("OSMI-1 (50 uM)_", well_Number)  & per_Well_Plate$condition[i] != paste0("WT Ctrl_", well_Number)){
    if(starting_Int <= 640){ 
    #print(paste0("CS",i))
    per_Well_Plate$conf_Location[i] = paste0("CS",starting_Int)
...
#Only activate deletion if excedding the compounds being tested:
if (length(rows_to_Delete) >= 1){
  delete_Rows <-per_Well_Plate[rows_to_Delete, ]
  per_Well_Plate <- per_Well_Plate[-rows_to_Delete, ] 
}
...
stats_Df <- data.frame("Plate" = paste(trial_Name, time_Point, sep = ""), "Well/Condition" = paste0("DMSO (0.1%)_", well_Number), "Average_foci_per_nuc" = mean_Nuc_Foci_DMSO, "Average_Cell_Count" = mean_Cell_DMSO, "Average_Cyto_Foci" = mean_Cyto_Foci_DMSO, "SB_Nuc_Foci" = NA, "SD_Nuc_Foci" = sd_Nuc_Foci_DMSO, "Z_prime_Nuc_Foci" = NA, "CV_Nuc_Foci" = CV_Nuc_Foci_DMSO, "Confidential_Name" = paste0("DMSO (0.1%)_", well_Number))
...
statistical_Analysis <- function(conditions){
  ...
  confidental <- unique(conditions$condition)
  ...
  temp_Dfs <- data.frame("Plate" = time_And_Plate, "Well/Condition" = well_Name, "Average_foci_per_nuc" = mean_Nuc_Foci, "Average_Cell_Count" = mean_Cell_Count, "Average_Cyto_Foci" = mean_Cyto_Foci, "SB_Nuc_Foci" = SB_Nuc_Foci, "SD_Nuc_Foci" = sd_Nuc_Foci, "Z_prime_Nuc_Foci" = z_prime_Nuc_Foci, "CV_Nuc_Foci" = CV_Nuc_Foci_Percent, "Confidential_Name" = confidental)
...
statistical_Analysis_compounds <- function(conditions){
  ...
  well_Name <- unique(conditions$Well_Location)
  confidental <- unique(conditions$conf_Location)
  ...
  temp_Dfs <- data.frame("Plate" = time_And_Plate, "Well/Condition" = well_Name, "Average_foci_per_nuc" = mean_Nuc_Foci, "Average_Cell_Count" = mean_Cell_Count, "Average_Cyto_Foci" = mean_Cyto_Foci, "SB_Nuc_Foci" = SB_Nuc_Foci, "SD_Nuc_Foci" = NA, "Z_prime_Nuc_Foci" = NA, "CV_Nuc_Foci" = NA, "Confidential_Name" = confidental)
...
x_min <- stats_Df[stats_Df$Well.Condition == paste0("DMSO (0.1%)_", well_Number), ]$Average_foci_per_nuc
x_max <- stats_Df[stats_Df$Well.Condition == paste0("WT Ctrl_", well_Number), ]$Average_foci_per_nuc
x_min2 <- stats_Df[stats_Df$Well.Condition == paste0("OSMI-1 (50 uM)_", well_Number), ]$Average_foci_per_nuc
...
Normalized_mean_Effect <- mean(stats_Df$Normalized_Effect)
Normalized_SD_Effect <- sd(stats_Df$Normalized_Effect)
...
normalized_Dist <- ggplot(stats_Df, aes(x = Normalized_Effect)) +
  ...
  annotate("text", x=97, y=15, label="WT Ctrl", angle=90, size = 3.5) +
  geom_vline(aes(xintercept = 0), linetype = 'dashed', size = 1) +
  annotate("text", x=-3, y=15, label="DMSO Ctrl", angle=90, size = 3.5) +
  
  geom_vline(aes(xintercept = mean_3SD), linetype = 'dashed', color = "red", size = 1) +
  annotate("text", x=mean_3SD - 3, y=15, label="(-) 3SD", angle=90, size = 3.5) +
  
  geom_vline(aes(xintercept = mean_Minus3SD), linetype = 'dashed', color = "red", size = 1) +
  annotate("text", x=mean_Minus3SD - 3, y=15, label="(+) 3SD", angle=90, size = 3.5) +
  
  labs(title = paste("Normalized Effect on MLF2-GFP Foci: ", time_And_Plate,  sep =""), y = "Count", x = "% Decrease in MLF2-GFP Foci") +
...
normalized_Dist
insertPlot(wb, sheet = figure_Worksheet, startRow = 2, startCol = 1, width = 6, height = 4)

raw_Distribution
insertPlot(wb, sheet = figure_Worksheet, startRow = 2, startCol = 9, width = 6, height = 4)
...
#Export the raw data and hit list:
...
```{r}
screen_Actives_Df <- data.frame(matrix(ncol = 8))
colnames(screen_Actives_Df) = c("Plate", "Well", "Normalized_SD", "Average_MLF2_Foci", "Average_Cell_Count", "Effect", "Viability", "Confidential_Name")

for (i in 1:length(combined_Stats_Df$Normalized_SD)){
  if (combined_Stats_Df$Normalized_SD[i] < -3.0 | combined_Stats_Df$Normalized_SD[i] > 3.0){
    screen_Active <- c(combined_Stats_Df$Plate[i], combined_Stats_Df$Well.Condition[i], combined_Stats_Df$Normalized_SD[i], combined_Stats_Df$Average_foci_per_nuc[i], combined_Stats_Df$Average_Cell_Count[i],  combined_Stats_Df$Normalized_Effect[i], combined_Stats_Df$Viability[i], combined_Stats_Df$Confidential_Name[i])
    print(screen_Active)
...
screen_Actives_Df_Subset <-  subset(screen_Actives_Df, grepl("^CS", Confidential_Name))
writeData(wb, "Putative Actives", screen_Actives_Df_Subset)


#Combine_Data:
putative_Actives_Df <- screen_Actives_Df
#putative_Actives_Df <- rbind(putative_Actives_Df, screen_Actives_Df)
# putative_Actives_Df2$Confidential_Name <- factor( putative_Actives_Df$Confidential_Name, levels = putative_Actives_Df$Confidential_Name)
putative_Actives_Df$Well <- factor(putative_Actives_Df$Well, levels = c(putative_Actives_Df$Well))



#Factor so that the graph shows wells based on their location in 384 well:
combined_Stats_Df$Well.Condition<- factor( combined_Stats_Df$Well.Condition, levels = combined_Stats_Df$Well.Condition)
...
normalized_Sd_Bar <- ggplot(data = combined_Stats_Df, aes(x=Well.Condition, y = Normalized_SD, fill = factor(ifelse(Normalized_SD < -3 | Normalized_SD > 3, "tofill", "nottofill")))) +
  ...
  labs(y = "Standardized Effect (#SD)", x = "Wells", title = "Selection of Screen Active Compounds (24h):") +
  scale_y_continuous(limits = c(-10, 10)) +
  scale_y_continuous(breaks=c(-6, -3, -1, 0, 1, 3, 6)) +
  geom_hline(yintercept = c(-3,3), color = "darkred", linetype = "dashed", size = 0.5) +
  geom_text_repel(data = filter(combined_Stats_Df, combined_Stats_Df$Normalized_SD < -3 | Normalized_SD > 3), aes(label = Confidential_Name), box.padding = 0.3, size = 3) +
  guides(fill="none")+
...
normalized_Sd_Bar
tiff("C:/Users/Schlieker Lab/Desktop/Dylan/Compound Screen/pilot_screen_9_1_23_dp/pilot_24h.tiff", units="in", width=10, height=10, res=300)
ggarrange(normalized_Sd_Bar, heights = c(6, 6))
dev.off()
...
normalized_Sd_Bar
insertPlot(wb, sheet = "Combined Analysis", startRow = 1, startCol = 1, width = 6, height = 6)


gathered_df <- pivot_longer(screen_Actives_Df_Subset, cols = c("Effect", "Viability"), 
...
putative_Actives_Fig <- ggplot(data = gathered_df, aes(x= Well, fill = efficacy, y = as.numeric(relative_score))) +
  geom_col(stat = "identity", position = position_dodge(0.6)) +
  scale_fill_manual(values = c("Effect" = "gray27", "Viability" = "darkred")) +
  theme_classic() +
  labs(y = "Relative Score (%)", x = "Wells", title = "Efficacy of Screen Active Compounds: (24h)") +
  theme(plot.title = element_text(size = 20)) +
  theme(axis.title = element_text(size = 20))+
  ...
  theme(axis.title.x=element_blank(),) +
  scale_x_discrete(labels = gathered_df$Confidential_Name[seq(1, length(gathered_df$Confidential_Name), by = 2)])

putative_Actives_Fig
insertPlot(wb, sheet = "Combined Analysis", startRow = 16, startCol = 1, width = 6, height = 4)


tiff("C:/Users/Schlieker Lab/Desktop/Dylan/Compound Screen/pilot_screen_9_1_23_dp/efficacypilot_24h.tiff", units="in", width=10, height=10, res=300)
ggarrange(putative_Actives_Fig, heights = c(6, 6))
dev.off()



##Use for poster/presentations:
#------------
Normalized_mean_Effect <- mean(combined_Stats_Df$Normalized_Effect)
Normalized_SD_Effect <- sd(combined_Stats_Df$Normalized_Effect)

mean_minus_3SD <- Normalized_mean_Effect - 3*Normalized_SD_Effect
mean_plus_3SD <- Normalized_mean_Effect + 3*Normalized_SD_Effect

raw_Distribution <- ggplot(combined_Stats_Df, aes(x = Average_foci_per_nuc )) +
  geom_histogram(color = "black", fill = "darkred", binwidth = 1) +
  geom_density(alpha = 0.6, fill = "darkgray" ) +
  labs(title = "Distribution of MLF2-GFP Foci (24h): ",  y = "Count", x = "MLF2-GFP Foci") +
  theme_classic()

normalized_Dist <- ggplot(combined_Stats_Df, aes(x = Normalized_Effect)) +
  geom_histogram(color = "black", fill = "darkred", binwidth = 2.75) +
  geom_density(alpha = 0.6, fill = "darkgray" ) +
  
  geom_vline(aes(xintercept = 100), linetype = 'dashed', size = 1) +
  annotate("text", x=96, y=25, label="WT Ctrl", angle=90, size = 5) +
  geom_vline(aes(xintercept = 0), linetype = 'dashed', size = 1) +
  annotate("text", x=-4, y=25, label="DMSO Ctrl", angle=90, size = 5) +
  
  geom_vline(aes(xintercept = mean_plus_3SD), linetype = 'dashed', color = "red", size = 1) +
  annotate("text", x=mean_plus_3SD - 4, y=25, label="(+) 3SD", angle=90, size = 5) +
  ...
  labs(title = "Normalized Effect on MLF2-GFP Foci (24h):", y = "Count", x = "% Decrease in MLF2-GFP Foci") +
...
# 
# ggplot(combined_Stats_Df) + 
#   aes(x="", y = Normalized_Effect)+
#   geom_boxplot(fill="blue")

normalized_Dist
tiff("C:/Users/Schlieker Lab/Desktop/Dylan/Compound Screen/pilot_screen_9_1_23_dp/distrpilot_24h.tiff", units="in", width=12, height=10, res=400)
ggarrange(normalized_Dist, heights = c(6, 6))
dev.off() 

normalized_Dist
tiff("C:/Users/Schlieker Lab/Desktop/Dylan/Compound Screen/pilot_screen_9_1_23_dp/rawdistrpilot_24h.tiff", units="in", width=12, height=10, res=400)
ggarrange(raw_Distribution, heights = c(6, 6))
dev.off()

raw_Distribution
insertPlot(wb, sheet = "Combined Analysis", startRow = 45, startCol = 1, width = 16, height = 4)

normalized_Dist
insertPlot(wb, sheet = "Combined Analysis", startRow = 45, startCol = 1, width = 6, height = 4)
...
# bcpc_query("50-07-7", from = "name")
# is.cas("50-07-7", verbose = getOption("verbose"))
# 
#  if (!requireNamespace("BiocManager", quietly=TRUE))
#      install.packages("BiocManager")
#  BiocManager::install("ChemmineR")
# 
# sdf1 <- "C:/Users/Schlieker Lab/Desktop/Dylan/Compound Screen/pilot_screen_9_1_23_dp/sdf_actives.sdf"
# data(sdf1); sdfset <- sdf1
# sdfset; view(sdfset[1:4])
# sdfset[[1]]
# 
# sdfset <- read.SDFset("http://faculty.ucr.edu/~tgirke/Documents/R_BioCond/Samples/sdfsample.sdf")
# ids<-loadSdf(conn,sdf1, function(sdfset) 
#                      data.frame(rings(sdfset,type="count",upper=6, arom=TRUE)) )
# library(ChemmineR)
# 
# sdfset <- read.SDFstr("C:/Users/Schlieker Lab/Desktop/Dylan/Compound Screen/pilot_screen_9_1_23_dp/sdf_actives.sdf")
# write.SDF(sdfstr[1:4], file="sub.sdf", sig=TRUE) 
# plot(sdfsample[1:4], print=FALSE) # Plots structures to R graphics device 
# 
# view(sdfset)
